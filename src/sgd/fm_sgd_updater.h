 ...
 */
class FMSGDUpdater : public Updater {
 public:
  FMSGDUpdater() {}
  virtual ~FMSGDUpdater() {}
  ...
  const FMSGDUpdaterParam& param() const { return param_; }

 private:
  /** \brief update w by FTRL */
  void UpdateW(real_t gw, FMSGDEntry* e);

  /** \brief update V by adagrad */
  void UpdateV(real_t const* gV, FMSGDEntry* e);

  /** \brief init V */
  void InitV(FMSGDEntry* e);

  /** \brief new w for a server */
  float new_w = 0;

  FMSGDUpdaterParam param_;
  std::unordered_map<feaid_t, FMSGDEntry> model_;
