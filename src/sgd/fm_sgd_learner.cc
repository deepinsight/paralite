...
KWArgs FMSGDLearner::Init(const KWArgs& kwargs) {
  // init tracker
...
void FMSGDLearner::RunScheduler() { 
...
void FMSGDLearner::RunEpoch(int epoch, int job_type, sgd::Progress* prog) {
  // progress merger
  tracker_->SetMonitor(
...
void FMSGDLearner::GetPos(const SArray<int>& len,
...
void FMSGDLearner::Process(const std::string& args, std::string* rets) {
...
void FMSGDLearner::IterateData(const sgd::Job& job, sgd::Progress* progress) {
...
real_t FMSGDLearner::EvaluatePenalty(const SArray<real_t>& weights,
